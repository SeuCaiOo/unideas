...
- **Duas telas que já compartilham composables continuam compartilhando quando ganham UI nova.** Se um elemento novo (linha, botão, bloco) é adicionado nos dois call sites, ele nasce como composable próprio em `screen/components/` do módulo (nunca `:uds`, se depender de modelo de domínio ou de `R.*` do módulo) e é chamado uma vez de cada tela — nunca colado inline e duplicado nas duas. Vale mesmo quando o elemento é pequeno (ex.: uma `Row` de poucos parâmetros) — duplicação nasce pequena. Confirmado o motivo na prática (2026-07-21, quando `HomeScreen` e a extinta `BrowseScreen` ainda coexistiam): duplicação de um `Row(Filters + ViewModeToggleButton)` entre as duas, extraída depois pro próprio módulo por já violar DRY/KISS na criação.
...
- **Testar debounce/`delay()` em ViewModel:** guardar a instância de `UnconfinedTestDispatcher()` num campo da classe de teste (não inline em `setUp()`), e avançar o tempo dela explicitamente via `testDispatcher.scheduler.advanceTimeBy(...)`/`advanceUntilIdle()` no teste — nunca depender do fim do `runTest` resolver o `delay()` sozinho implicitamente. Confirmado o jeito difícil (#133): esse comportamento implícito é inconsistente entre testes (às vezes "resolve" o `delay` pendente, às vezes não), causando falso positivo/negativo dependendo de detalhe não documentado do dispatcher.
- **Testar `onCleared()` de ViewModel:** é `protected` em `androidx.lifecycle.ViewModel`, não chamável direto do teste. Usar um `androidx.lifecycle.ViewModelStore` real — `store.put("key", vm)` seguido de `store.clear()` — pra disparar de verdade. Confirmado (#133): checar `Job.isActive` dentro de `onCleared()` não funciona pra decidir se havia trabalho pendente, porque `viewModelScope` já é cancelado *antes* de `onCleared()` rodar (a cancelação do escopo acontece primeiro); usar uma flag (`var` simples, não atrelada ao estado da coroutine) pra rastrear isso.
