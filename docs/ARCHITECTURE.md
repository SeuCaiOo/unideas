...
:feature:home  →  :core:backup  (#211 — hospeda BackupSyncViewModel + bottom sheets de restore/disable-sync
                    direto na Listagem; mesma exceção de dependência do :feature:settings)
...
- Lista agrupada por data (#212): `BackupDateGroup` (`Esta semana`/`Semana passada`/`Este mês`/`Mês passado`/`Mais antigos`, arquivo próprio no pacote raiz de `:core:backup`, função pura `LocalDate.toBackupDateGroup()`) — sem grupos "Hoje"/"Ontem", já cobertos pela label individual de cada item. Semana com início no domingo, por aritmética de data simples (não `WeekFields.weekOfWeekBasedYear()`, que quebra em viradas de ano). `BackupBottomSheet` renderiza um `GroupHeader` (`:uds`) por grupo dentro do mesmo `LazyColumn`, mesmo padrão de header-por-grupo já usado no agrupamento por seção da Home.
...
- **Gatilho (#215): observer central no banco, não navegação/resume de tela.** `AutoBackupTrigger` (interface em `:domain`, impl `AutoBackupTriggerImpl` em `:core:backup` via `AutoBackupScheduler`/`AutoBackupWorker`, `WorkManager` `enqueueUniqueWork(..., ExistingWorkPolicy.KEEP, ...)`, mesmo padrão do `ReminderCheckWorker`) hoje é chamado só de `AutoBackupDataObserver` — um `Room.InvalidationTracker.Observer` registrado em `UnideasDatabase` no startup (Koin `single(createdAtStart = true)`), cobrindo as 5 tabelas rastreadas (`items`/`sections`/`tags`/`item_tag`/`item_completion_history`); qualquer escrita real numa delas dispara `triggerNow()`. Substituiu duas gerações anteriores: hookar em `HomeUseCase.refreshReminders()` (pull-to-refresh/resume, #192/#193) perdia mutações fora da Listagem (ex: editar tag na Config Screen, filha do Detalhe, não da Home); uma tentativa intermediária de propagar uma flag `hasChanges` manualmente por `NavBackStackEntry.savedStateHandle` tela a tela teve o mesmo problema e foi revertida. Observar o banco direto cobre qualquer mutação futura sem wiring manual por tela. **Resiliência a falha (#212):** o `WorkRequest` exige `Constraints.setRequiredNetworkType(NetworkType.CONNECTED)` — sem rede, o WorkManager segura o job na fila (persistido, sobrevive a reiniciar o app) em vez de rodar e falhar silenciosamente; `AutoBackupWorker.doWork()` propaga o `Result` real de `PerformAutoBackupUseCase` (`Result.retry()` numa falha real, aciona o backoff do WorkManager), em vez do antigo `Result.success()` incondicional que escondia qualquer erro.
- **Regra de substituição (slot único, nunca empilha):** `PerformAutoBackupUseCase` — checa `isEnabled()` e, desde #211, também `GetBackupSyncStateUseCase` (pula silenciosamente se `Desynced` — ver desincronismo, abaixo), faz upload via `BackupUseCase.upload(account)`, grava o novo `fileId` como rastreado, e só então apaga o `fileId` anterior (falha ao apagar é só logada via `Timber.w`, não desfaz o upload que já teve sucesso).
...
- **UI:** `Switch` inline dentro de `ConnectedBackupContent` (`BackupBottomSheet.kt`, não virou função própria pra não estourar `TooManyFunctions` do arquivo), ligado a `BackupUiState.isAutoBackupEnabled`/`BackupEvent.OnAutoBackupToggled`. Tag "· Automático" somente-leitura no subtítulo do item de Backup em `SettingsScreen`, sem precisar abrir o sheet pra saber o estado. Desligar a flag emite `BackupUiAction.ShowSnackbar` (`backup_auto_backup_disabled`) — antes do #211 não havia nenhum sinal visual de que o toggle tinha desligado de verdade.

### Desincronismo multi-aparelho e restore/disable-sync (#209/#210/#211)

Motivação: o backup automático acima é via de mão única (só sobe) — em mais de um aparelho, cada um sobrescrevia o slot remoto sem saber que havia uma versão mais nova de outro aparelho.

- `GetBackupSyncStateUseCase` (injeta `BuildDriveServiceUseCase`/`ListBackupsUseCase`/`AutoBackupRepository` direto, não a facade `BackupUseCase` — cada um é usado sozinho, sem composição extra que justifique a facade) compara o `fileId` do backup mais recente no Drive contra o rastreado localmente (`AutoBackupRepository.getTrackedFileId()`), retornando `BackupSyncState` (`NoRemoteBackup`/`Synced`/`Desynced(remoteBackup)`).
- `GetConfirmedBackupSyncStateUseCase` evita falso positivo: uma única leitura do Drive pode pegar o próprio upload deste aparelho ainda não refletido no `fileId` rastreado localmente (race entre upload e leitura). Reexecuta `GetBackupSyncStateUseCase` depois de um delay (1500ms) e só considera desincronizado se a segunda leitura confirmar.
- `BackupSyncViewModel` (`core/backup/viewmodel/sync/`) hospeda o fluxo — evento `OnSyncCheckRequested`: só roda se `AutoBackupSettingsUseCase.isEnabled()` e há conta logada; se confirmado desincronizado, abre `BackupSyncDialogState.RestorePrompt` (bottom sheet fixo, não-dismissível). Confirmar restore (`OnRestoreConfirmClicked` → `BackupUseCase.restore` → `BackupSyncUiAction.RestoreCompleted`, tratado com `Context.restartApplication()`, mesmo motivo do restore manual) ou recusar (`OnRestoreDeclineClicked` → `DisableSyncConfirm`, segundo bottom sheet fixo perguntando se quer desligar o auto-backup; recusar esse volta pro primeiro, sem deixar num limbo "flag ligada mas decidiu não sincronizar"). Confirmar desligar (`OnDisableSyncConfirmClicked`) desliga a flag e emite o mesmo `ShowSnackbar` de aviso citado acima.
- `syncCheckCompleted: StateFlow<Boolean>` sinaliza quando a checagem termina (com ou sem desincronismo). `:feature:home`'s `HomeScreen` hospeda `BackupSyncViewModel` (mesma exceção de dependência que `:feature:settings` já tem pra `:core:backup`) e dispara `OnSyncCheckRequested` no mesmo `DisposableEffect`/observer de lifecycle que já dispara `HomeEvent.OnScreenResumed` (cobre abertura fria + retomada do Detalhe). `HomeContent` usa `syncCheckCompleted` pra duas coisas: (1) segurar a lista (`HomeBody`) no loading até a checagem terminar, mesmo com a query local já resolvida — sem isso a lista aparecia e só segundos depois (leitura do Drive) surgia o bottom sheet de desincronismo por cima. **Só na primeira vez por processo** (`ColdStartSyncGate`, mesmo padrão em memória do `ColdStartPriorityPrompt` logo abaixo): a checagem dispara de novo em todo `ON_RESUME`, não só voltando do Detalhe — sem esse gate, a lista voltava a piscar pro loading toda vez que o usuário voltava de Configurações/Tags/Seções pra Home, mesmo sem nada de backup ter mudado (bug real, corrigido em #212); (2) gatear o auto-abrir do `PriorityBottomSheet` (`ColdStartPriorityPrompt`) pra só avaliar depois que o sync resolveu **e** nenhum snackbar está visível (`SnackbarHostState.currentSnackbarData != null`) — sem isso o bottom sheet de Prioridades abria por cima do snackbar de auto-backup desligado, cobrindo-o antes que desse pra ler. Esse gate do prompt de Prioridades roda só fora de `LocalInspectionMode.current` — sem isso o preview do Compose tenta abrir o `PriorityBottomSheet` de verdade (que resolve `koinViewModel()`) e crasha com Koin não inicializado no processo de preview.
- **Fluxo com auto-backup desligado (backup manual, ou ligar a flag):** `BackupViewModel` reaproveita `GetBackupSyncStateUseCase` (não a versão "confirmada" — aqui é ação explícita do usuário, não checagem em background) antes de `OnBackupClick`/`OnAutoBackupToggled(true)`; se desincronizado, emite `BackupUiAction.ShowOverwriteConfirm(pending: PendingBackupOverwrite)` (aviso simples de sobrescrita, não o fluxo de 2 bottom sheets acima) — só prossegue com upload/enable após `OnOverwriteConfirmClicked`.
- Pacotes: `BackupViewModel`/`BackupSyncViewModel` e seus tipos (`Event`/`UiState`/`UiAction`) vivem em subpacotes próprios (`viewmodel/backup/`, `viewmodel/sync/`), não mais um `viewmodel/` único — mesmo padrão de um-subpacote-por-tela já usado em `feature/items`/`feature/home`.
