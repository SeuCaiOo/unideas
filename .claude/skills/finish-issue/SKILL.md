...
**Note:** the unideas board only has `Todo` / `In Progress` / `Done` (no `In Review`) — the card is deliberately left in "In Progress" here. It gets swept to "Done" automatically the next time `/start-feature` runs (step 0), once the issue is closed by the merge.
