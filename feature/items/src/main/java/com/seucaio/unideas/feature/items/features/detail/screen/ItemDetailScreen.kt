...
/**
 * Metadata rendered via `:uds` native [TitleSubtitle] (title + optional description), [MetaRow],
 * [MetaChipsRow] (read-only tag display) and [TextBadge] (item type), plus [DueDateRow] — a
 * feature-local composable mirroring `MetaRow`'s shape but slotting in `:uds`'s native `DueBadge`
 * for the urgency dot+color `MetaRow`'s plain-text value can't express. `UnideasTopBar`,
 * `UnideasLoadingContent`/`UnideasErrorContent` and `DeleteConfirmationDialog` stay legacy — no
...
private fun ItemDetailBody(state: ItemDetailUiState.Success, modifier: Modifier = Modifier) {
    ...
    Column(
        ...
        DueDateRow(item = item, isLast = !showRecurrence)
