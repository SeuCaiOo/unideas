...
- Debug builds only: Settings → **Depuração** → **Design system** opens `:uds`'s `ComponentGallery` full-screen, grouped by category (typography, buttons, chips, inputs, lists, navigation, panels, feedback) under collapsible headers — the fastest way to eyeball a token/component change without hunting for the screen that happens to use it.
