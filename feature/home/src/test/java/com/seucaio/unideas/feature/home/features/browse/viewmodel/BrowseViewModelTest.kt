...
class BrowseViewModelTest {
    ...
    private fun viewModel() = BrowseViewModel(homeUseCase, getSectionsAndTags)
    ...
    fun `when OnTabChanged should switch the active tab and reload the tab list`() = runTest {
        ...
        vm.itemsState.test {
            ...
            vm.onEvent(BrowseEvent.OnTabChanged(ItemType.NOTE))
            val state = awaitItem()
    ...
    fun `when OnSectionFilterChanged should reload the tab list filtered by section`() = runTest {
        ...
        vm.itemsState.test {
            awaitItem()
            vm.onEvent(BrowseEvent.OnSectionFilterChanged(7L))
            awaitItem()
    ...
    fun `when OnTagFilterToggled should reload the tab list filtered by the toggled tag`() = runTest {
        ...
        vm.itemsState.test {
            awaitItem()
            vm.onEvent(BrowseEvent.OnTagFilterToggled(9L))
            awaitItem()
        }
    ...
    fun `when OnViewModeChanged should switch the view mode`() = runTest {
        ...
        vm.onEvent(BrowseEvent.OnViewModeChanged(ItemsViewMode.GRID))
    ...
    fun `when OnSectionPinToggled succeeds should reload sections reflecting the new pin state`() = runTest {
        ...
        vm.onEvent(BrowseEvent.OnSectionPinToggled(section.id, true))
    ...
    fun `when OnSectionPinToggled fails should emit ShowError`() = runTest {
        ...
        vm.uiAction.test {
            vm.onEvent(BrowseEvent.OnSectionPinToggled(1L, true))
            assertEquals(BrowseUiAction.ShowError("boom"), awaitItem())
        }
    }
    ...
    fun `when OnCompleteClicked for a known item should call HomeUseCase's complete`() = runTest {
        ...
        vm.onEvent(BrowseEvent.OnCompleteClicked(1L))
    ...
    fun `when hasAnyItem throws should emit Error`() = runTest {
        ...
        vm.uiState.test {
            assertEquals(BrowseUiState.Error(R.string.home_load_error), awaitItem())
        }
    }
    ...
    fun `when OnRetryClicked after an error should retry and succeed`() = runTest {
        ...
        vm.uiState.test {
            assertEquals(BrowseUiState.Error(R.string.home_load_error), awaitItem())
            vm.onEvent(BrowseEvent.OnRetryClicked)
            ...
            val state = awaitItem() as BrowseUiState.Success
    ...
    fun `when OnItemClicked should emit NavigateToDetail with the item id`() = runTest {
        ...
        vm.uiAction.test {
            vm.onEvent(BrowseEvent.OnItemClicked(42L))
            assertEquals(BrowseUiAction.NavigateToDetail(42L), awaitItem())
        }
    }
    ...
    fun `when OnAddClicked should emit NavigateToAddItem with the chosen type`() = runTest {
        ...
        vm.uiAction.test {
            vm.onEvent(BrowseEvent.OnAddClicked(ItemType.NOTE))
            assertEquals(BrowseUiAction.NavigateToAddItem(ItemType.NOTE), awaitItem())
        }
    }
}
