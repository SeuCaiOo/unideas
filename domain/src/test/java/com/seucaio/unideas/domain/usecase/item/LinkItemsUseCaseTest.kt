...
class LinkItemsUseCaseTest {

    private val repository: ItemLinkRepository = mockk()
    private val useCase = LinkItemsUseCase(repository)

    @Test
    fun `invoke delegates the exact ids to the repository`() = runTest {
        coEvery { repository.linkItems(1L, 2L) } returns Unit

        val result = useCase(1L, 2L)

        assertEquals(Result.success(Unit), result)
        coVerify(exactly = 1) { repository.linkItems(1L, 2L) }
    }

    @Test
    fun `invoke wraps a repository failure in Result`() = runTest {
        val error = IllegalStateException("boom")
        coEvery { repository.linkItems(1L, 2L) } throws error
