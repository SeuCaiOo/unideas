...
- `:core:backup` — Google Drive backup/restore, self-contained (scoped `GoogleSignIn` + Drive API, not Firebase Auth). `GoogleAuthUseCase`/`GoogleAuthRepository` also expose `signOut()` (#183) — session-only, no Firebase Auth involved. Optional automatic backup (#193, off by default): `AutoBackupTrigger` (domain interface, mirrors `ReminderRefreshTrigger`) fires only in response to an actual data mutation, never on plain navigation/pull-to-refresh (#215 — the earlier rule, firing on every Home resume/refresh, produced backups with no real content change, which broke cross-device desync detection). `HomeUseCase.refreshReminders()` itself no longer touches the trigger; `HomeUseCase.triggerAutoBackup()` is a separate call, invoked by `HomeViewModel` only from: (a) `OnScreenResumed(hasChanges)` when the flag reports a save happened during the just-closed Detail/Config visit (passed back via `NavBackStackEntry.savedStateHandle`, `Constants.ITEM_SAVED_RESULT_KEY`, set in `AppNavHost` right before `popBackStack()`), and (b) list-level pin/complete handlers, since the Listagem already knows those mutated data directly. Pull-to-refresh never triggers it. Single-slot replace (`PerformAutoBackupUseCase` tracks one `fileId`, deletes the previous after a successful upload); the tracked slot shows up tagged "automatic" in the manually-managed backup list (#184) instead of duplicating an entry.
