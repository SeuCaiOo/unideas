...
- `:core:backup` — Google Drive backup/restore, self-contained (scoped `GoogleSignIn` + Drive API, not Firebase Auth). `GoogleAuthUseCase`/`GoogleAuthRepository` also expose `signOut()` (#183) — session-only, no Firebase Auth involved. Optional automatic backup (#193, off by default): `AutoBackupTrigger` (domain interface, mirrors `ReminderRefreshTrigger`) fires from `AutoBackupDataObserver` (#215) — a Room `InvalidationTracker.Observer` registered on `UnideasDatabase` at app startup (Koin `single(createdAtStart = true)`) for all 5 tracked tables (`items`, `sections`, `tags`, `item_tag`, `item_completion_history`), calling `triggerNow()` on any write. A DB-level observer replaced an earlier attempt at manually threading a `hasChanges` flag through nav callbacks/`NavBackStackEntry.savedStateHandle` — that approach missed real mutation paths outside the screens it was wired for (e.g. editing tags on the Config Screen, since it's a child of Detail, not Home, in the back stack) and would keep missing any future one; observing the database itself needs no per-screen wiring and covers every mutation path automatically. The observer doesn't gate on the auto-backup preference itself — that check stays where it always was, inside `PerformAutoBackupUseCase` (`isEnabled()`), so a disabled flag still makes every triggered run a no-op. Single-slot replace (`PerformAutoBackupUseCase` tracks one `fileId`, deletes the previous after a successful upload); the tracked slot shows up tagged "automatic" in the manually-managed backup list (#184) instead of duplicating an entry.
