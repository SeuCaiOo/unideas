...
class DeleteBackupUseCaseTest {
    ...
    private val useCase = DeleteBackupUseCase(repository)

    @Test
    fun `invoke delegates the file id to the repository`() = runTest {
        coEvery { repository.deleteBackup(driveService, "file-id-1") } returns Result.success(Unit)

        val result = useCase(driveService, "file-id-1")

        assertTrue(result.isSuccess)
        coVerify(exactly = 1) { repository.deleteBackup(driveService, "file-id-1") }
    }

    @Test
    fun `invoke propagates a failure from the repository`() = runTest {
        coEvery { repository.deleteBackup(driveService, "file-id-1") } returns
