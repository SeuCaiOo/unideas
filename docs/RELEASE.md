...
`prepare_release.yml` (workflow_dispatch, run manually from `main`) bumps `versionCode`/`versionName` in `app/build.gradle.kts`, commits, tags (`vX.Y.Z`), then in parallel: calls `release_build.yml`, which builds a signed release APK, creates a draft GitHub Release and uploads to Firebase App Distribution (group `alpha-testers`); and opens a mergeback PR (`main` → `dev`, auto-merge armed) so `dev` never drifts behind the version-bump commit — same manual pattern used before automation (PRs #90, #112, #123), now automatic so it isn't forgotten before starting the next feature branch. `release-drafter.yml` keeps a draft changelog up to date on every push to `main`, categorized by label via `.github/changelog-drafter.yml` — `release_build.yml` reuses that draft's body for the real release's notes (then deletes the draft) instead of GitHub's native auto-generated notes, to avoid a redundant "New Contributors" section on a single-dev repo.
